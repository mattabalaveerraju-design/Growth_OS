      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-[13.5px] text-ink-soft">Daily, weekly, monthly, quarterly, yearly. Goal → Project → Task.</p>
          </div>
          <button
            type="button"
            onClick={openNew}
              ...
              {goalsByType[type].length ? (
                <div className="space-y-3">
                  {goalsByType[type].map((goal) => (
                      <div key={goal.id} className="rounded-[14px] border border-border/60 p-4 hover:border-border transition-colors">
          ...
          <div className="grid gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <select
                ...
                onChange={(event) => setFormState((prev) => ({ ...prev, type: event.target.value as GoalType }))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <Input
                ...
                onChange={(event) => setFormState((prev) => ({ ...prev, targetValue: Number(event.target.value) }))}
                placeholder="Target value"
              />
              <Input
                type="number"
                value={formState.currentValue || 0}
                onChange={(event) => setFormState((prev) => ({ ...prev, currentValue: Number(event.target.value) }))}
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Are you sure you want to delete this goal?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
