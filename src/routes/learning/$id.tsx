...
import { useLearningStore } from "@/stores/useGrowthStores";

export const Route = createFileRoute("/learning/$id")({
  head: () => ({ meta: [{ title: "Learning Detail — GrowthOS" }] }),
  component: LearningDetailPage,
});

function LearningDetailPage() {
  const { id } = useParams({ from: "/learning/$id" });
  ...
  const localItem = useMemo(
    () => learning.find((entry) => entry.id === id) ?? null,
    [learning, id],
  );
  ...
  const item =
    ...
    (localItem
      ? {
          ...
          description: localItem.notes,
          category: localItem.category,
          notes: localItem.notes,
          source: localItem.source,
          confidence: localItem.confidence,
          timeHours: localItem.timeHours,
          createdAt: localItem.date,
          updatedAt: localItem.date,
          lastOpenedAt: localItem.date,
        }
      : null);
  ...
  const persistCardProgress = async (updates: {
    ...
    if (hasSupabaseConfig) {
      await updateCloudCardProgress("learning", item.id, payload);
      return;
    }
  ...
  const handleAddFiles = async (incoming: FileList | null) => {
    ...
    for (const [index, entry] of entries.entries()) {
      ...
      if (!file) continue;
      const saved = await uploadCloudFile("learning", item.id, file);
      if (saved) {
        const cloudFiles = await listCloudFiles("learning", item.id);
        setFiles(cloudFiles.map(mapCloudFileToResourceFileItem));
      }
    }
  };
  ...
  if (isLoading) {
    return (
      <AppShell title="Learning detail">
      ...
      <AppShell title="Learning detail">
        <div className="space-y-4">
          ...
          <EmptyState
            ...
            description="This learning resource is unavailable right now."
          />
        </div>
      </AppShell>
    );
