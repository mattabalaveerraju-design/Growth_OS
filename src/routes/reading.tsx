...
import { useInterviewStore, VaultItem } from "@/stores/useGrowthStores";
...
export const Route = createFileRoute("/reading")({
  head: () => ({ meta: [{ title: "Interview Prep — GrowthOS" }] }),
...
function InterviewPrepPage() {
  ...
  const deleteInterviewNote = useInterviewStore((state) => state.deleteInterviewNote);
  ...
  useEffect(() => {
    ...
    let active = true;
    listCloudFiles("reading", activeNote.id).then((files) => {
      if (!active) return;
  ...
  const visibleNotes = useMemo(
    () => (cloudEnabled ? cloudItems.map(mapCloudCardToVault) : interviewNotes),
    [cloudEnabled, cloudItems, interviewNotes],
  );

  const filteredNotes = useMemo(
    () =>
      visibleNotes.filter((item) =>
  ...
  const resourceItems = useMemo<ResourceItem[]>(() => {
    ...
    return interviewNotes.map((item) => ({
      id: item.id,
      module: "reading" as const,
      title: item.title,
  ...
  const handleSave = async () => {
    ...
    if (cloudEnabled) {
      const saved = await upsertCloudCard("reading", {
        id: activeNote?.id,
        title,
      ...
      if (saved) {
        ...
        if (selectedFile) {
          await uploadCloudFile("reading", saved.id, selectedFile);
          const files = await listCloudFiles("reading", saved.id);
          setExistingFiles(files);
        }
        setDialogOpen(false);
        setSelectedFile(null);
    ...
    if (activeNote) {
      updateInterviewNote(activeNote.id, { ...formState, title });
    } else {
      addInterviewNote({ ...formState, title });
    }
    setDialogOpen(false);
  };
  ...
  const handleDelete = async () => {
    ...
    if (cloudEnabled) {
      await deleteCloudCard("reading", deleteTarget);
    ...
    deleteInterviewNote(deleteTarget);
    setDeleteOpen(false);
    setDeleteTarget("");
  };
  ...
  const removeAttachment = async (fileId: string) => {
    ...
    const files = await listCloudFiles("reading", activeNote.id);
    setExistingFiles(files);
  };

  const handleFavoriteToggle = async (item: ResourceItem) => {
    if (cloudEnabled) {
      const updated = await updateCloudCardProgress("reading", item.id, {
        favorite: !item.favorite,
    ...
    updateInterviewNote(item.id, { favorite: !item.favorite });
  };
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-[13.5px] text-ink-soft">
              Interview questions, notes, and preparation resources
            </p>
          </div>
          <button
            type="button"
        ...
        >
          ...
          {filteredNotes.length ? (
            <ResourceGrid
              ...
              onOpen={(item) => navigate({ to: "/reading/$id", params: { id: item.id } })}
            ...
            <div className="rounded-3xl border border-border p-6 text-sm text-ink-soft text-center">
              No interview prep yet. Start building your preparation guide.
              <div className="mt-4">
                <button
                  type="button"
                  onClick={openNew}
        ...
        <DialogContent className="w-[calc(100vw-24px)] max-w-2xl overflow-hidden p-0 sm:rounded-[24px]">
          <div className="flex max-h-[calc(100vh-24px)] flex-col">
            <DialogHeader className="px-4 py-4 sm:px-6">
              <DialogTitle>{activeNote ? "Edit Topic" : "Add Topic"}</DialogTitle>
            </DialogHeader>
              ...
              <div className="grid gap-4">
                ...
                {formState.type === "note" ? (
                  <Textarea
                    ...
                    placeholder="Your answer, notes, or preparation"
                  />
                ) : (
                  <>
                    <Input
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this interview prep topic?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
