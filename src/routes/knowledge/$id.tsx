...
import { useVaultStore } from "@/stores/useGrowthStores";

export const Route = createFileRoute("/knowledge/$id")({
  head: () => ({ meta: [{ title: "Knowledge Detail — GrowthOS" }] }),
...
function KnowledgeDetailPage() {
  ...
  const localItem = useMemo(() => vault.find((entry) => entry.id === id) ?? null, [vault, id]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (hasSupabaseConfig) {
        ...
        const cards = await listCloudCards("knowledge");
        const found = cards.find((card) => card.id === id);
        if (!active) return;
        setCloudItem(found ? mapCloudCardToResourceItem(found, "knowledge") : null);
        if (found) {
          const cloudFiles = await listCloudFiles("knowledge", found.id);
          if (!active) return;
  ...
  const persistCardProgress = async (updates: {
    ...
    if (hasSupabaseConfig) {
      await updateCloudCardProgress("knowledge", item.id, payload);
      return;
    }
  ...
  const handleAddFiles = async (incoming: FileList | null) => {
    ...
    for (const [index] of entries.entries()) {
      ...
      if (!file) continue;
      const saved = await uploadCloudFile("knowledge", item.id, file);
      if (saved) {
        const cloudFiles = await listCloudFiles("knowledge", item.id);
        setFiles(cloudFiles.map(mapCloudFileToResourceFileItem));
      }
    }
  };
  ...
  if (isLoading) {
    return (
      <AppShell title="Knowledge detail">
      ...
      <AppShell title="Knowledge detail">
        <div className="space-y-4">
          ...
          <EmptyState
            ...
            description="This knowledge resource is unavailable right now."
          />
        </div>
      </AppShell>
    );
