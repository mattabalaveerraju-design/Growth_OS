...
const productivityData = [
  { d: "Mon", v: 72 }, { d: "Tue", v: 78 }, { d: "Wed", v: 75 },
  { d: "Thu", v: 82 }, { d: "Fri", v: 85 }, { d: "Sat", v: 89 }, { d: "Sun", v: 87 },
];

const consistencyData = productivityData.map((p, i) => ({ d: p.d, v: 70 + i * 3 + Math.sin(i) * 2 }));
...
function AnalyticsPage() {
  ...
  const interviewRate = applications.length ? Math.round((applications.filter((item) => item.status === "Interview").length / applications.length) * 100) : 0;
  const offerRate = applications.length ? Math.round((applications.filter((item) => item.status === "Offer").length / applications.length) * 100) : 0;
    ...
    <AppShell title="Analytics">
      <div className="space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="font-display text-[28px] font-semibold tracking-[-0.025em]">Analytics</h1>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-0.5 rounded-[10px] bg-card border border-border h-9 px-1">
              <button className="h-7 w-7 grid place-items-center text-ink-soft hover:text-ink"><ChevronLeft className="h-4 w-4" /></button>
              <span className="h-7 px-3 text-[12.5px] font-medium text-ink flex items-center">May 17 – 24, 2026</span>
              <button className="h-7 w-7 grid place-items-center text-ink-soft hover:text-ink"><ChevronRight className="h-4 w-4" /></button>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-5">
          ...
          >
            <div className="flex items-baseline justify-between">
              <div>
                <div className="text-[11.5px] font-semibold tracking-[0.12em] text-ink-soft/80 uppercase">Hours Overview</div>
                <div className="mt-2 flex items-baseline gap-3">
                  <span className="font-display text-[32px] font-semibold tracking-tight">24h 30m</span>
            ...
            <div className="h-[260px] mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={hoursData} margin={{ left: -16, right: 0, top: 8, bottom: 0 }}>
                  <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fill: "oklch(0.5 0.015 265)", fontSize: 11 }} />
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: "oklch(0.5 0.015 265)", fontSize: 11 }} />
                  <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid oklch(0.92 0.006 95)", fontSize: 12, background: "white" }} />
                  <Bar dataKey="Work" stackId="a" fill="oklch(0.52 0.22 275)" radius={[0, 0, 0, 0]} />
                  <Bar dataKey="Learning" stackId="a" fill="oklch(0.62 0.25 295)" />
                  <Bar dataKey="Personal" stackId="a" fill="oklch(0.78 0.15 75)" />
                  <Bar dataKey="Exercise" stackId="a" fill="oklch(0.72 0.16 155)" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </motion.div>
          ...
          >
            <div className="text-center">
              <div className="text-[11.5px] font-semibold tracking-[0.12em] text-ink-soft/80 uppercase">Productivity Score</div>
              <div className="font-display text-[68px] font-semibold tracking-tight leading-none mt-3 bg-gradient-to-br from-primary to-violet bg-clip-text text-transparent">87%</div>
              <div className="text-[13px] text-ink-soft mt-2">Great job!</div>
              <div className="h-[100px] mt-4 -mx-4">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={productivityData}>
                    <Line type="monotone" dataKey="v" stroke="oklch(0.52 0.22 275)" strokeWidth={2.5} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <motion.div
            ...
            className="card-soft p-6"
          >
            <div className="text-[11.5px] font-semibold tracking-[0.12em] text-ink-soft/80 uppercase">Top Categories</div>
            <div className="flex items-center gap-5 mt-4">
              <div className="h-32 w-32 shrink-0">
                <ResponsiveContainer>
                  <PieChart>
                    <Pie data={categories} dataKey="value" innerRadius={36} outerRadius={56} strokeWidth={0}>
          ...
          <motion.div
            ...
            className="card-soft p-6"
          >
            <div className="text-[11.5px] font-semibold tracking-[0.12em] text-ink-soft/80 uppercase">Weekly Goal Progress</div>
            ...
            <div className="mt-4 h-2 w-full rounded-full bg-muted overflow-hidden">
              <div className="h-full rounded-full bg-gradient-to-r from-primary to-violet" style={{ width: "75%" }} />
            </div>
              ...
              {Array.from({ length: 35 }, (_, i) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      background: intensity < 0.3 ? "oklch(0.94 0.005 95)" : intensity < 0.6 ? "oklch(0.85 0.08 275)" : "oklch(0.55 0.2 275)",
                    }}
                  />
                );
              })}
          ...
          <motion.div
            initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }}
            className="card-soft p-6"
          >
            <div className="text-[11.5px] font-semibold tracking-[0.12em] text-ink-soft/80 uppercase">Consistency</div>
            <div className="mt-3 flex items-baseline gap-2">
            ...
            <div className="h-[140px] mt-4 -mx-4">
              <ResponsiveContainer>
                <LineChart data={consistencyData}>
                  <Line type="monotone" dataKey="v" stroke="oklch(0.72 0.16 155)" strokeWidth={2.5} dot={{ r: 3, fill: "oklch(0.72 0.16 155)" }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </motion.div>
...
function Legend2() {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      {items.map((i) => (
        <span key={i.label} className="inline-flex items-center gap-1.5 text-[11.5px] text-ink-soft">
